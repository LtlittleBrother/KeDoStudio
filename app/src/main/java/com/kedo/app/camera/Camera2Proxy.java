...
//            Size largest = Collections.max(Arrays.asList(map.getOutputSizes(ImageFormat.JPEG)), new CompareSizesByArea());
            Size largest = new Size(width,height);
...
//            mPreviewSize = chooseOptimalSize(map.getOutputSizes(SurfaceTexture.class), width, height, largest);
    ...
    public void setPreviewSurface(SurfaceTexture surfaceTexture,int w,int h) {
        if (mPreviewSize == null ){
            surfaceTexture.setDefaultBufferSize(h, w);
        }else {
...
//    }
    ...
    private Size getSuitableSize(Size[] sizes, int width, int height, Size pictureSize) {
        ...
        for (int i = 0; i < sizes.length; i++) {
            Size size = sizes[i];
//            LogUtils.debugInfo(TAG, "picture size " + i +" :"+ size.getWidth() + "*" + size.getHeight());
            // 先判断比例是否相等
...
//        LogUtils.debugInfo(TAG, "handleZoom: maxZoom: " + maxZoom);
...
//            mZoom--;
//        }
//        LogUtils.debugInfo(TAG, "handleZoom: mZoom: " + mZoom);
...
//        LogUtils.debugInfo(TAG, "handleZoom: cropW: " + cropW + ", cropH: " + cropH);
...
//                return;
//            }
//            LogUtils.debugInfo(TAG, "process: CONTROL_AF_STATE: " + state);
//            if (state == CaptureResult.CONTROL_AF_STATE_FOCUSED_LOCKED || state == CaptureResult
//                    .CONTROL_AF_STATE_NOT_FOCUSED_LOCKED) {
//                LogUtils.debugInfo(TAG, "process: start normal preview");
