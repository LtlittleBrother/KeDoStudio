...
//        if (checkDebuggerConnected) return true;
    ...
    public static String getMCC() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            //暂时没有更好的办法去做兼容
            return Settings.System.getString(Utils.getApp().getContentResolver(), Settings.Secure.ANDROID_ID);
        } else {
            String mcc = "460";
            TelephonyManager telManager = (TelephonyManager) Utils.getApp().getSystemService(Context.TELEPHONY_SERVICE);
            if (telManager == null) {
                return mcc;
            }
    ...
    public static String getMNC() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {//暂时没有更好的办法去做兼容
            return Settings.System.getString(Utils.getApp().getContentResolver(), Settings.Secure.ANDROID_ID);
        } else {
            String mnc = "00";
            TelephonyManager telManager = (TelephonyManager) Utils.getApp().getSystemService(Context.TELEPHONY_SERVICE);
