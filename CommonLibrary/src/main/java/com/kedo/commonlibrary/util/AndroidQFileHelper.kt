...
object AndroidQFileHelper {
     ...
     * 兼容android10的文件获取方式
     */
    private fun getSdcardCacheFilePath(): String {
        return File(appContext.getExternalFilesDir("cache"),"").path
    }

    private fun getDefaultPath(): String {
        return getSdcardCacheFilePath() + separator + "可咚" + separator
    }

    fun getRecordVideoPath(): String{
        return getSdcardCacheFilePath().plus("RecordVideo").plus(separator)
    }
