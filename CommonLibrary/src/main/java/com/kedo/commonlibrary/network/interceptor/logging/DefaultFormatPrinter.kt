...
import com.kedo.commonlibrary.util.CharacterHandler.Companion.xmlFormat
import com.kedo.commonlibrary.util.LogUtils
import com.kedo.commonlibrary.network.interceptor.logging.LogInterceptor.Companion.isJson
import com.kedo.commonlibrary.network.interceptor.logging.LogInterceptor.Companion.isXml
...
class DefaultFormatPrinter : FormatPrinter {
    ...
    override fun printJsonResponse(
        ...
        var bodyStringNew = bodyString
        bodyStringNew =
            when {
                isJson(contentType) -> jsonFormat(bodyStringNew!!)
                isXml(
                    contentType
                ) -> xmlFormat(bodyStringNew)
                else -> bodyStringNew
            }
        val responseBody =
            LINE_SEPARATOR!! + BODY_TAG + LINE_SEPARATOR + bodyStringNew
